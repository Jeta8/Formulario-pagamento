...
document.querySelector(".nome-titular-input").oninput = () => {
  document.querySelector(".card-holder-name").innerText =
    document.querySelector(".nome-titular-input").value;
};

document.querySelector(".mes-exp-input").oninput = () => {
  document.querySelector(".exp-month").innerText =
    document.querySelector(".mes-exp-input").value;
};

document.querySelector(".ano-exp-input").oninput = () => {
  document.querySelector(".exp-year").innerText =
    document.querySelector(".ano-exp-input").value;
};
...
let bandeiraFinal; // Bandeira detectada
let valido = true; // Indica se a bandeira foi detectada ou não
let nomeValido = false; // Verifica se o nome inserido é válido

document.getElementById("btn-enviar").value = "Finalizar Pagamento"; // Define o 'context text' do botão (texto do botão de submit)

// Notificações de erro
var spanInvalido = document.getElementsByClassName("invalid");
var spanInvalidoSelect = document.getElementsByClassName("invalid-select");
const inputs = document.querySelectorAll("input");

// Esta variável só é satisfeita quando todos os inputs e selects são corretamente válidados
let isFormValid = false;

function filtro_Nome(nome) {
  nome = document.getElementById("nome-titular").value;
  letras_regex = /^[A-Za-zÀ-ÖØ-öø-ÿ ]+$/;

  if (nome.match(letras_regex)) {
    nomeValido = true;
    return;
  } else {
    nomeValido = false;
...
function finalizar_pagamento() {
  ...
  var nomeTitular = document.getElementById("nome-titular").value.toUpperCase();
  ...
  if (
    (nomeTitular == "") |
    ((nomeTitular.length <= 8) | (nomeValido != true))
  ) {
    spanInvalido[1].setAttribute("style", "visibility: visible");
    inputs[1].setAttribute("style", "border-color: red");
    isFormValid = false;
    return;
  } else if (nomeTitular.length >= 6 && nomeValido == true) {
    isFormValid = true;
    spanInvalido[1].setAttribute("style", "visibility: hidden");
    inputs[1].setAttribute("style", "border-color: black");
  }

  if ((mesExpiracao == "month") | (mesExpiracao == null)) {
    spanInvalidoSelect[0].setAttribute("style", "visibility: visible");
    isFormValid = false;
    console.log(mesExpiracao.value);
    return;
  } else {
    isFormValid = true;
    spanInvalidoSelect[0].setAttribute("style", "visibility: hidden");
  }

  if ((anoExpiracao == "year") | (anoExpiracao == null)) {
    spanInvalidoSelect[1].setAttribute("style", "visibility: visible");
    isFormValid = false;
    return;
  } else {
    isFormValid = true;
    spanInvalidoSelect[1].setAttribute("style", "visibility: hidden");
  }
